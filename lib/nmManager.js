...
module.exports = {
    getNetworkManagerState: function(callback){
        return nmDbus.NetworkManager.getState()
            .then(function(state){
                if (callback)
                    return callback(null, Enumerator.NM_STATE.translate(state))
                return Enumerator.NM_STATE.translate(state);
            });
    },
